...
export default function Post() {
	...
	const blog = data.blogs.find(item => utility.parser.linkify(item.title) == post);
				...
				{blog && (
					<Stack gap={"md"}>
						<AspectRatio ratio={1920 / 1080}>
							<Image src={blog.image} alt={blog.title} radius={"xs"} />
						</AspectRatio>
						<Container size={"sm"}>
							<Stack gap={"md"}>
								<Stack gap={"xs"}>
									<Title order={2}>{blog.title}</Title>
									<Group gap={"xs"} fz={"xs"} fw={500} c={"pri.6"}>
										<Text component="p" fz={"inherit"} fw={"inherit"}>
											{blog.date}
										</Text>{" "}
										{/* /
										<Text
								...
								<Stack gap={"lg"} fz={"sm"}>
									...
									{blog.desc.quotes && (
										<Blockquote cite={blog.desc.quotes[0].cite}>
										...
										<List spacing={"xs"} size={"sm"} listStyleType="none">
											{blog.desc.list.map(item => (
												<ListItem key={item.title}>
													<Text component="span" inherit fw={500} c={"pri.6"}>
														{item.title}
													</Text>
													: {item.desc}
												</ListItem>
